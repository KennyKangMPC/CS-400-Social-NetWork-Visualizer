...
public class Main extends Application {
	...
	Button helperButton = new Button("Help");
	...
	private HBox helperBox() {
		HBox UIButtons = new HBox();
		helperButton.setPrefSize(100, 50);
		helperButton.setStyle("-fx-background-color: MediumSeaGreen;" + "-fx-font-size: 10pt");
		UIButtons.getChildren().addAll(helperButton);
		return UIButtons;
	}
	
	private void helper(String title) {
		Stage help = new Stage();
		BorderPane root = new BorderPane();
		help.initModality(Modality.APPLICATION_MODAL);
		help.setTitle(title);
		help.setMinWidth(1000);
		help.setMinHeight(750);
		
		buttons.setHgap(7.5);
		buttons.setVgap(15);
		Button addUserBox = new Button("Add (with one textfield)");
		Button removeUserBox = new Button("Remove (with one textfield)");
		Button centerUserBox = new Button("Set Center");
		Button addFriendShipBox = new Button("Add (with two textfields)");
		Button removeFriendShipBox = new Button("Remove (with two textfields)");
		Button uploadFileBox = new Button("Upload file");
		Button writeFileBox = new Button("Write to File");
		
		Text text1 = new Text(500, 30, "Guidelines");
		text1.setFill(Color.CHOCOLATE);
		text1.setFont(Font.font(java.awt.Font.SERIF, 25));
		Text upload = new Text(300, 78, "Click on this button to upload a file");
		Text write = new Text(300, 158, "Click on this button to write a file");
		Text center = new Text(300, 238, "Click on this button to get another tree with that user as the center");
		Text add1 = new Text(300, 318, "Click on this button and double click on canvas to add user");
		Text remove1 = new Text(300, 398, "Click on this button to remove a user");
		Text add2 = new Text(300, 478, "Click on this button to add a bond between two people");
		Text remove2 = new Text(300, 558, "Click on this button to remove the bond between two people");
		
		conversion(uploadFileBox, 4);
		conversion(writeFileBox, 6);
		conversion(centerUserBox, 8);
		conversion(addUserBox, 10);
		conversion(removeUserBox, 12);
		conversion(addFriendShipBox, 14);
		conversion(removeFriendShipBox, 16);
		

		root.setLeft(buttons);
		root.getChildren().addAll(text1, upload, write, center, add1, remove1, add2, remove2);
		Scene scene = new Scene(root, 200, 200);
		help.setScene(scene);
		help.show();
	}
	
	private void conversion(Button button, int i) {
		HBox UIButtons = new HBox();
		button.setPrefSize(250, 50);
		button.setStyle("-fx-background-color: MediumSeaGreen;" + "-fx-font-size: 10pt");
		UIButtons.getChildren().addAll(button);
		UIButtons.setSpacing(5.0);
		Text yeah = new Text("hi");
		buttons.add(button, 1, i);
	}
	...
	private GridPane LeftPanelUI() {
		...
		HBox helperBox = helperBox();
		helperBox.setSpacing(5.0);
